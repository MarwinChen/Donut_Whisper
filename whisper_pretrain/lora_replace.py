...
def replace_one_conv_layer(model, name, module, lora_alpha, lora_r):
    ...
    # 将原来的nn.Linear层的权重和偏置复制给新的lora.Linear层
    new_module.conv.weight.data = weight.data.clone()
    if bias is not None:
        new_module.conv.bias.data = bias.data.clone()
...
def replace_multi_layers(model, lora_alpha=1, lora_r=16):
    for name, module in list(model.named_modules()):
        if name.startswith('donut_model.encoder') or name.startswith('whisper_model.decoder') or name.startswith('whisper_model.encoder'):
            if isinstance(module, torch.nn.Linear) and any(s in name for s in ["query", "key", "value", "out"]):
                replace_one_linear_layer(model, name, module, lora_alpha, lora_r)

def replace_whisper_layers(model, lora_alpha=1, lora_r=16):
    for name, module in list(model.named_modules()):
        if name.startswith('whisper_model.decoder'):
            if isinstance(module, torch.nn.Linear) and any(s in name for s in ["query", "key", "value", "out"]):
                replace_one_linear_layer(model, name, module, lora_alpha, lora_r)
