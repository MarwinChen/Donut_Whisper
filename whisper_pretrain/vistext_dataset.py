...
class CustomDataset(Dataset):
    def __init__(self, input_features_path, labels_path, device):
    ...
    def __getitem__(self, idx):
        return torch.load(self.input_features_paths[idx], map_location=self.device), torch.load(self.labels_paths[idx], map_location=self.device)