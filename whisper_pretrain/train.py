...
logfile = "/home/srt11/data_8849/model/transmodel/whisper_pre_3/logfile.txt"
logfile_simp = "/home/srt11/data_8849/model/transmodel/whisper_pre_3/logfile_simp.txt"
expdir = "/home/srt11/data_8849/model/transmodel/whisper_pre_3"
scheduler = 'warmuplr'
accumgrad = 10
decay_pct = 0.2
warmup_pct = 0.0
...
# model = torch.load("/home/srt11/data_8849/model/transmodel/whisper_pre_1/model.acc.best.pth", weights_only=False)
...
# for name, param in model.named_parameters():
#     if  'whisper_model.encoder' in name:
#         param.requires_grad = False
...
input_features_train = '/hdd/srt11/Dataset_large_input_features/train'
labels_train = '/hdd/srt11/Dataset/train_data/labels'

train_set = CustomDataset(input_features_path=input_features_train, labels_path=labels_train)
...
input_features_dev = '/hdd/srt11/Dataset_large_input_features/validation'
labels_dev = '/hdd/srt11/Dataset/val_data/labels'

dev_set = CustomDataset(input_features_path=input_features_dev, labels_path=labels_dev)
...
CER = 0
with torch.no_grad():
    for idx, (input_features, labels) in enumerate(tqdm(dev_data, unit="batch")):          
        ...
        # Forward pass
        logits = model(data_whisper = input_features, labels = target)
        ...
        if idx % 10 == 0 and idx > 0:
            ...
            logging("{} out of {} finished | WER: {} | CER:{} | Loss: {}".format(
                idx, len(dev_data), wer, cer, totalloss/10), logfile)
            totalloss = 0
            WER = wer
            CER = cer
        if  WER > 0.25:
    ...
    logging("[epoch {}] Total WER: {}, CER: {}".format(0, WER, CER), logfile)
    logging("[epoch {}] Total WER: {}, CER: {}".format(0, WER, CER), logfile_simp)

with open(f"out_2/{0}_out.txt", "w", encoding="utf-8") as file:
    for r in o_list:
        file.write(r)
        file.write("\n")
with open(f"out_2/{0}_ref.txt", "w", encoding="utf-8") as file:
    for r in l_list:
        file.write(r)
        file.write("\n")
...
for epoch in range(num_epochs):
    ...
    with tqdm(train_data, unit="batch") as tepoch:
        for idx, batch in enumerate(tepoch):
            ...
            # target[target == -100] = tokenizer.eot
            ...
            labell = pad_sequence([y[1:] for y in oritarget], batch_first=True, padding_value=-100).to(model.module.device)
            logits = model(data_whisper = input_features, labels=target)
            # output = torch.log_softmax(logits[:, sotlen-1:], dim=-1)
    ...
    with open(f"out_2/{epoch+1}_out.txt", "w", encoding="utf-8") as file:
        for r in o_list:
            file.write(r)
            file.write("\n")
    with open(f"out_2/{epoch+1}_ref.txt", "w", encoding="utf-8") as file:
        for r in l_list:
            file.write(r)
            file.write("\n")
